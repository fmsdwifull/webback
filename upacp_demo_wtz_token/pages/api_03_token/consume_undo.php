<form class="api-form" method="post" action="demo/api_03_token/Form_6_8_ConsumeUndo.php" target="_blank">
<p>
<label>商户号：</label>
...
<p>
<label>订单发送时间：</label>
<input id="txnTime" type="text" pattern="\d{14}" name="txnTime" placeholder="订单发送时间" value="<?php echo date('YmdHis')?>" title="取北京时间，YYYYMMDDhhmmss格式。" required="required"/>
<p>
<label>商户订单号：</label>
<input id="orderId" type="text" pattern="[0-9a-zA-Z]{8,32}" name="orderId" placeholder="商户订单号" value="<?php echo date('YmdHis')?>" title="8-32位数字字母，自行定义内容。" required="required"/>
</p>
<p>
<label>交易金额：</label>
...
<hr />
<?php include $_SERVER ['DOCUMENT_ROOT'] . '/upacp_demo_wtz_token/pages/more_faq.php';?>
</div>