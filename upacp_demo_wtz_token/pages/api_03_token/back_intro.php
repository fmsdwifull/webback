...
<b>注：因配样例中测试商户号777290058110097银联后台配置的是 借记卡是 前台开通的权限，信用卡是后台开通权限，故本“无跳转token-商户侧开通”样例只能使用信用卡号测试。</b><br><br>
交易流程：<br>
<b>首次：开通短信交易-> 后台开通交易（如果开通交易配置验证短信先做开通短信交易）;开通查询（根据后台开通的订单号orderId查询获取token)<br>
     消费： 使用token号做消费短信交易(如果商户号配置验证短信), 使用token号做消费交易<br>
    后续可以对消费交易做退货，消费撤销交易<br>
  后续可以对token号做更新标记，解除标记交易<br>
 可以对消费，退货，消费撤销进行交易状态查询交易<br>
 可以通过对账文件下载交易获取商户的对账文件</b><br><br>
 
后台开通：同步应答，同步应答获取token号<br><br>
开通查询：后台交易，无后台通知<br>
            如果后台开通没有收到后台通知无法获取token号，可以调用次交易获取token号<br><br>
            
开通短信：后台交易，无后台通知<br>
       上送手机号，卡号等发送消费短信<br><br>
